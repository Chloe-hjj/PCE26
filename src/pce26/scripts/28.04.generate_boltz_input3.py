...
def main():
    ...
    # Analysis for pdb2 (Acidic source)
    sasa_df2 = compute_sasa(pdb2)
    dist_df2 = compute_distance_to_center(pdb2)

    sasa_df2['residue_family'] = sasa_df2['residue_type'].map(amino_acid_properties)

    # Merge and normalize distances for pdb2
    full_df2 = pd.merge(dist_df2, sasa_df2)

    full_df2['distance_mean'] = full_df2['distance'].mean()
    full_df2['distance_norm'] = full_df2['distance'] - full_df2['distance_mean']
    full_df2['location'] = full_df2['distance_norm'].apply(lambda x: 'outer' if x > 0 else 'inner')

    # Identify the best GLU or ASP
    best_acid= (full_df2.query('location == "outer" and (residue_type == "GLU" or residue_type == "ASP")').sort_values('sasa_total', ascending=False).head(1))
